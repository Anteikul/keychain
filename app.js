...
function openItem(data=null){
  ...
  dlg.querySelector('[data-dialog-title]').textContent=data?'Upravit přístup':'Přidat přístup';
...
document.querySelectorAll('[data-reveal]').forEach(b=>b.onclick=()=>{const c=b.parentNode.querySelector('code');const shown=b.textContent==='Skrýt';c.textContent=shown?'••••••••••':c.dataset.secret;b.textContent=shown?'Zobrazit':'Skrýt'});
async function copyText(value){
  if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(value);return}
  const area=document.createElement('textarea');area.value=value;area.setAttribute('readonly','');area.style.cssText='position:fixed;opacity:0;pointer-events:none';document.body.appendChild(area);area.select();area.setSelectionRange(0,area.value.length);const ok=document.execCommand('copy');area.remove();if(!ok)throw new Error('Kopírování selhalo')
}
async function copyButton(b,value){const old=b.textContent;try{await copyText(value);b.textContent='Zkopírováno'}catch{b.textContent='Označte ručně'}setTimeout(()=>b.textContent=old,1600)}
